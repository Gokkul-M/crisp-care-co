      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab} className="animate-fade-in" style={{ animationDelay: '200ms' }}>
        ...
        <TabsContent value="active" className="space-y-4">
          {offers.filter(o => o.status === "active").map((offer, index) => (
            <Card key={offer.id} className="service-card hover-lift animate-fade-in p-4" style={{ animationDelay: `${index * 100}ms` }}>
        ...
        <TabsContent value="used" className="space-y-4">
          {offers.filter(o => o.status === "used").map((offer, index) => (
            <Card key={offer.id} className="service-card opacity-75 animate-fade-in p-4" style={{ animationDelay: `${index * 100}ms` }}>
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center space-x-3">
